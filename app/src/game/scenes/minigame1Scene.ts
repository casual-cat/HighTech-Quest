...
export default class minigame1Scene extends Phaser.Scene {
    ...
    private tasks: { id: number; sprite: Phaser.GameObjects.Image; column: string }[] = [];
    ...
    preload(): void {
        this.load.image("minigame1-background", "/assets/game/level3/minigames/hierarchy-tasks/minigame1-background.png");
        this.load.image("todo", "/assets/game/level3/minigames/hierarchy-tasks/toDo.png");
        this.load.image("inProgress", "/assets/game/level3/minigames/hierarchy-tasks/inProgress.png");
        this.load.image("done", "/assets/game/level3/minigames/hierarchy-tasks/done.png");
        this.load.image("task1", "/assets/game/level3/minigames/hierarchy-tasks/task1.png");
        this.load.image("task2", "/assets/game/level3/minigames/hierarchy-tasks/task2.png");
        this.load.image("task3", "/assets/game/level3/minigames/hierarchy-tasks/task3.png");
        this.load.image("task4", "/assets/game/level3/minigames/hierarchy-tasks/task4.png");
        this.load.image("task5", "/assets/game/level3/minigames/hierarchy-tasks/task5.png");
        this.load.image("task6", "/assets/game/level3/minigames/hierarchy-tasks/task6.png");
        this.load.image("task7", "/assets/game/level3/minigames/hierarchy-tasks/task7.png");
        this.load.image("task8", "/assets/game/level3/minigames/hierarchy-tasks/task8.png");
        this.load.image("task9", "/assets/game/level3/minigames/hierarchy-tasks/task9.png");
    }
    ...
    create(): void {
        ...
        this.add.text(width / 2, buttonY, "Submit", {
            fontSize: "24px",
            color: "#ffffff"
        }).setOrigin(0.5);
    }
    ...
    private shuffleAndDistributeTasks(): void {
        ...
        for (let i = 0; i < 9; i++) {
            ...
            const taskSprite = this.add.image(this.columns[columnIndex].x, taskY, `task${taskId}`)
                .setInteractive({ useHandCursor: true });
    ...
    private makeTaskDraggable(taskSprite: Phaser.GameObjects.Image, taskId: number): void {
        ...
        taskSprite.on('drag', (pointer: Phaser.Input.Pointer, dragX: number, dragY: number) => {
            taskSprite.x = dragX;
            taskSprite.y = dragY;
        });
        ...
        taskSprite.on('dragend', (pointer: Phaser.Input.Pointer) => {
            ...
            if (droppedSlot) {
                const taskInSlot = this.getTaskInSlot(droppedSlot.column, droppedSlot.slotIndex);
                if (taskInSlot && taskInSlot.id !== taskId) {
                    this.animateSwap(taskId, taskInSlot.id);
                } else if (!taskInSlot) {
                    this.animateMoveToSlot(taskId, droppedSlot.column, droppedSlot.slotIndex);
                } else {
    ...
    private getSlotAtPosition(x: number, y: number): { column: string; slotIndex: number } | null {
        const { height } = this.scale;
    ...
    private getTaskInSlot(columnName: string, slotIndex: number): { id: number; sprite: Phaser.GameObjects.Image; column: string } | null {
    ...
    private moveTaskToSlot(taskId: number, columnName: string, slotIndex: number): void {
    ...
    private getSlotPosition(columnName: string, slotIndex: number): { x: number; y: number } {
        const { height } = this.scale;
        const column = this.columns.find(c => c.name === columnName);
    ...
    private getTaskIndexInColumn(taskId: number): { columnName: string; slotIndex: number } | null {
        for (const column of this.columns) {
    ...
    private animateSwap(taskId1: number, taskId2: number): void {
        ...
        const targetPosFor1 = this.getSlotPosition(task2Idx.columnName, task2Idx.slotIndex);
        const targetPosFor2 = this.getSlotPosition(task1Idx.columnName, task1Idx.slotIndex);
    ...
    private animateMoveToSlot(taskId: number, columnName: string, slotIndex: number): void {
    ...
    private isCorrect(): boolean {
        for (const correctColumn of this.correctColumns) {
            ...
            if (currentColumn.tasks.length !== correctColumn.tasks.length) return false;
